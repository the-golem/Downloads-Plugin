...
$pagination = '';
if ($pages > 1) for ($i = 1; $i <= $pages; $i++) $pagination .= '<a href="'.$fullURL.'&pg='.$i.'">'.($i==$page ? '<strong>'.$i.'</strong>' : $i ).'</a> ';
if (!empty($pagination)) $pagination = '<div style="text-align:center;">'.($page != 1 ? '<span style="float:left;"><a href="'.$fullURL.'&pg=1">&lt;&lt;</a> <a href="'.$fullURL.'&pg='.($page-1).'">&lt;</a></span> ' : '').'<span class="pages">'.$pagination.'</span>'.($page != $pages ? ' <span style="float:right;"><a href="'.$fullURL.'&pg='.($page+1).'">&gt;</a> <a href="'.$fullURL.'&pg='.$pages.'">&gt;&gt;</a></span>' : '').'</div>';
...
?>
	<tr class="<?php echo odd_even(); ?>">
		...
		<td>
			<a href="/download-file/<?php echo $download->id.'/'.$download->filename;?>" target="_blank" title="<?php echo $download->filename;?>"><img src="<?php echo PLUGINS_URI;?>/downloads/images/icon-open.png" alt="view icon" title="View"></a>
